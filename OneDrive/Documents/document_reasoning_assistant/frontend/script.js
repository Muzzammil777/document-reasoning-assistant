...
class DocumentReasoningApp {
    ...
    bindEvents() {
        ...
        analysisButton.addEventListener('click', this.navigateToAnalysis.bind(this));
    ...
    async createSession() {
        try {
            const response = await fetch(`${this.apiBaseUrl}/session`, { method: 'POST' });
    ...
    displayQueryResult(query, result) {
        // Store current query and result for analysis page
    ...
    navigateToAnalysis() {
        if (!this.currentQuery || !this.currentResult) {
            this.showError('No analysis data available. Please ask a question first.');
            return;
        }
        
        // Store analysis data in session storage
        const analysisData = {
        ...
        try {
            ...
            this.showError('Failed to navigate to analysis page. Please try again.');
        }
    }
